      ...
      <section className="relative overflow-hidden bg-brand" style={{ paddingTop: '10rem', paddingBottom: '4rem' }}>
        ...
        <div className="relative max-w-[68rem] mx-auto px-[60px] max-lg:px-6 max-md:px-4 text-center">
          ...
          <h1 className="text-stone font-extrabold tracking-tight mb-4" style={{ fontSize: 'clamp(2rem, 4vw, 3rem)', lineHeight: '1.1' }}>
            State Pool Contractor Insurance Requirements
          </h1>
          <p className="text-cream font-light mx-auto" style={{ fontSize: '1.1rem', lineHeight: '1.6', maxWidth: '600px' }}>
            Select a state to see contractor licensing requirements, workers compensation rules, minimum insurance requirements, and important state-specific regulations for pool contractors.
          </p>
        </div>
      </section>
          ...
          <div className="max-w-4xl mx-auto mb-12">
            ...
            <div className="flex flex-wrap gap-2">
              {stateRequirements.map((s) => (
                <button key={s.abbr} onClick={() => setSelectedState(s.abbr)} className={`rounded-[1rem] px-3 py-2 font-bold cursor-pointer ${selectedState === s.abbr ? 'bg-gold text-brand border-2 border-gold' : 'bg-white/40 text-brand border-2 border-ash hover:border-blue-dark'}`} style={{ fontSize: '0.75rem', transition: 'all 0.24s', minWidth: '52px' }}>
                  {s.abbr}
                </button>
              ))}
            </div>
            ...
            <div className="max-w-3xl mx-auto">
              <div className="border-2 border-ash rounded-[2rem] p-8 bg-white/40">
                <div className="flex items-center justify-between mb-8 flex-wrap gap-4">
                  ...
                  <div className="flex gap-3">
                    <Link href={`/states/${result.slug}/`} className="inline-flex items-center border-2 border-brand text-brand rounded-[2rem] font-semibold uppercase tracking-[0.12em] hover:bg-brand hover:text-stone no-underline" style={{ padding: '0.5rem 1.2rem', fontSize: '0.65rem', transition: 'all 0.24s' }}>Full State Page</Link>
                ...
                <div className="grid sm:grid-cols-2 gap-4 mb-6">
                  <div className="border border-ash rounded-[1rem] p-4">
                    <p className="text-brand/50 uppercase tracking-[0.1em] font-bold mb-1" style={{ fontSize: '0.65rem' }}>Recommended Minimum GL</p>
                  ...
                  <div className="border border-ash rounded-[1rem] p-4">
                    ...
                    <p className="text-brand font-bold" style={{ fontSize: '1rem' }}>
                      {result.wcRequired ? (result.wcMonopolistic ? (<span className="text-gold">Required — Monopolistic State Fund</span>) : (<span>Required</span>)) : (<span className="text-blue-dark">Voluntary (Texas)</span>)}
                    </p>
                  </div>
                  <div className="border border-ash rounded-[1rem] p-4">
                    <p className="text-brand/50 uppercase tracking-[0.1em] font-bold mb-1" style={{ fontSize: '0.65rem' }}>Licensing Agency</p>
              ...
              <div className="grid sm:grid-cols-2 gap-4 mt-6">
                <Link href={`/states/${result.slug}/`} className="block border-2 border-ash rounded-[2rem] p-5 hover:border-blue-dark no-underline" style={{ transition: 'all 0.24s' }}>
                  ...
                  <span className="text-brand font-bold block" style={{ fontSize: '0.9rem' }}>Pool Contractor Insurance in {result.state} &rarr;</span>
                </Link>
                  ...
                  <table className="w-full" style={{ fontSize: '0.8rem' }}>
                    <thead>
                      <tr className="bg-brand text-stone">
                        ...
                        <th className="text-left p-3 font-bold uppercase tracking-[0.1em]" style={{ fontSize: '0.7rem' }}>Min. GL</th>
